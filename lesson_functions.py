...
def color_hist(image, nbins=32):#, bins_range=(0, 256)):
    ...
    return hist_features #rhist, ghist, bhist, bin_centers, hist_features
...
def extract_features(imgs, color_space='RGB', spatial_size=(32, 32),
    ...
    for file in imgs:
        ...
        image = mpimg.imread(file) #training images are all png format so they are scaled 0to1
        #         image = image * 255
        #         image = image.astype(np.uint8)
        # apply color conversion if other than 'RGB'
        # remember that cv2.cvtColor() will return a 0 to 255 image.
        if color_space != 'RGB':
            if color_space == 'HSV':
...
def slide_window(img, x_start_stop=[None, None], y_start_stop=[None, None],
                 xy_window=(96, 96), xy_overlap=(0.75, 0.75)):     #64 64
